...
COMMENT ON COLUMN "public"."invoices"."last_reminder_sent_at" IS 'When last payment reminder was sent';



COMMENT ON COLUMN "public"."invoices"."reminder_count" IS 'How many reminders have been sent for this invoice';
...
COMMENT ON TABLE "public"."time_entries" IS 'Time tracking entries for billable/non-billable work. No longer linked to tasks.';



COMMENT ON COLUMN "public"."time_entries"."task_id" IS 'Deprecated: Task system removed, kept for backward compatibility';



COMMENT ON COLUMN "public"."time_entries"."invoice_id" IS 'Optional: Link time entry directly to invoice';



COMMENT ON COLUMN "public"."time_entries"."client_id" IS 'Required: Which client this time is for';
...
CREATE POLICY "client_portal_access_anon_select" ON "public"."client_portal_access" FOR SELECT TO "anon" USING ((("is_active" = true) AND (("expires_at" IS NULL) OR ("expires_at" > "now"()))));



CREATE POLICY "client_portal_access_anon_update" ON "public"."client_portal_access" FOR UPDATE TO "anon" USING ((("is_active" = true) AND (("expires_at" IS NULL) OR ("expires_at" > "now"())))) WITH CHECK ((("is_active" = true) AND (("expires_at" IS NULL) OR ("expires_at" > "now"()))));
...
CREATE POLICY "clients_anon_portal_access" ON "public"."clients" FOR SELECT TO "anon" USING (("id" IN ( SELECT "client_portal_access"."client_id"
   FROM "public"."client_portal_access"
  WHERE (("client_portal_access"."is_active" = true) AND (("client_portal_access"."expires_at" IS NULL) OR ("client_portal_access"."expires_at" > "now"()))))));
...
CREATE POLICY "invoices_anon_portal_access" ON "public"."invoices" FOR SELECT TO "anon" USING (("client_id" IN ( SELECT "client_portal_access"."client_id"
   FROM "public"."client_portal_access"
  WHERE (("client_portal_access"."is_active" = true) AND (("client_portal_access"."expires_at" IS NULL) OR ("client_portal_access"."expires_at" > "now"()))))));
...
CREATE POLICY "tenants_anon_portal_access" ON "public"."tenants" FOR SELECT TO "anon" USING (("id" IN ( SELECT "c"."tenant_id"
   FROM ("public"."clients" "c"
     JOIN "public"."client_portal_access" "cpa" ON (("cpa"."client_id" = "c"."id")))
  WHERE (("cpa"."is_active" = true) AND (("cpa"."expires_at" IS NULL) OR ("cpa"."expires_at" > "now"()))))));
