...
export function SLAMonitor({ onManageRules }: SLAMonitorProps) {
  ...
  const calculateSLAStatuses = (
    ...
    return tickets.map((ticket) => {
      ...
      const timeSinceCreated =
        (now.getTime() - createdAt.getTime()) / (1000 * 60 * 60); // hours
      ...
      const rule =
        ...
        rules.find(
          (r) => r.client_id === null && r.ticket_priority === ticket.priority
        );

      let responseProgress = 0;
      let resolutionProgress = 0;
      let responseSLAStatus: "compliant" | "at_risk" | "breached" | "n/a" =
        "n/a";
      let resolutionSLAStatus: "compliant" | "at_risk" | "breached" | "n/a" =
        "n/a";
      ...
      if (rule?.response_time_hours) {
        responseProgress = Math.min(
          100,
          (timeSinceCreated / rule.response_time_hours) * 100
        );
      ...
      if (rule?.resolution_time_hours) {
        if (ticket.status === "resolved" || ticket.status === "closed") {
          ...
          const resolutionTime =
            (new Date(ticket.updated_at).getTime() - createdAt.getTime()) /
            (1000 * 60 * 60);
          resolutionSLAStatus =
            resolutionTime <= rule.resolution_time_hours
              ? "compliant"
              : "breached";
          resolutionProgress = 100;
        } else {
          resolutionProgress = Math.min(
            100,
            (timeSinceCreated / rule.resolution_time_hours) * 100
          );
  ...
  const getOverallSLAStatus = (
    status: SLAStatus
  ): "compliant" | "at_risk" | "breached" | "n/a" => {
    const statuses = [
      status.response_sla_status,
      status.resolution_sla_status,
    ].filter((s) => s !== "n/a");
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">
                              {status.time_to_response
                                ? `Responded in ${formatTimeRemaining(
                                    status.time_to_response
                                  )}`
                                : `${formatTimeRemaining(
                                    status.response_time_hours -
                                      status.time_since_created
                                  )} remaining`}
                            </span>
                          </div>
                          <Progress
                            ...
                            className={`h-2 ${
                              status.response_sla_status === "breached"
                                ? "bg-red-100"
                                : status.response_sla_status === "at_risk"
                                ? "bg-yellow-100"
                                : "bg-green-100"
                            }`}
                          />
                          ...
                          <Progress
                            ...
                            className={`h-2 ${
                              status.resolution_sla_status === "breached"
                                ? "bg-red-100"
                                : status.resolution_sla_status === "at_risk"
                                ? "bg-yellow-100"
                                : "bg-green-100"
                            }`}
                          />
