...
import { queueInvoiceReminder } from "@/lib/reminders";
...
interface InvoicesState {
  ...
  processDueRecurringInvoices: (tenantId: string) => Promise<{ error?: string }>;
  fetchInvoiceStats: (
    tenantId: string,
...
export const useInvoicesStore = create<InvoicesState>((set, get) => ({
  ...
  createInvoice: async (tenantId: string, invoiceData: CreateInvoiceForm) => {
    try {
      ...
      if (invoice.due_date) {
        queueInvoiceReminder(invoice.id, invoice.due_date);
      }
  ...
  scheduleRecurringInvoice: async (id: string, recurrence: RecurrenceRule) => {
    try {
      ...
      set((state) => ({
        invoices: state.invoices.map((inv) =>
          inv.id === id ? { ...inv, recurrence_rule: recurrence, next_run_at: nextRunAt } : inv
        ),
      }));

      notify.success("Recurring invoice scheduled");
      return {};
    } catch (error: any) {
      console.error("Error scheduling recurring invoice:", error);
      notify.error(error.message || "Failed to schedule recurring invoice");
      return { error: error.message || "Failed to schedule recurring invoice" };
    }
  },

  processDueRecurringInvoices: async (tenantId: string) => {
    try {
      ...
      if (error) throw error;
      notify.success("Processed recurring invoices");
      return {};
    } catch (error: any) {
      console.error("Error processing recurring invoices:", error);
      notify.error(error.message || "Failed to process recurring invoices");
      return { error: error.message || "Failed to process recurring invoices" };
    }
  },

  fetchInvoiceStats: async (tenantId: string, filters?: StatsFilters) => {
    try {
      let query = supabase
        .from("invoices")
      ...
      (data || []).forEach((inv) => {
        ...
        revenueByMonthMap[key] = (revenueByMonthMap[key] || 0) + inv.total_amount;
      });

      const revenueByMonth = Object.keys(revenueByMonthMap)
        .sort()
        .map((month) => ({ month, total: revenueByMonthMap[month] }));

      set({
        invoiceStats: { totalRevenue, outstandingAmount, revenueByMonth },
      });
    } catch (error) {
      console.error("Error fetching invoice stats:", error);
      notify.error("Failed to fetch invoice stats");
    }
  },
}));
